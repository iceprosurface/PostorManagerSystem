  ...
  */
	...
	appModule.factory('ipcService', ['$http', '$q', function ($http, $q) {
		...
		return {
			...
			"positionPages": function positionPages() {
				var defer = $q.defer();
			...
			"unnoticedOrders": function unnoticedOrders(page) {
				var defer = $q.defer();
				$http.post('/admin/edit/query', {
	...
	//用户订单修改控制器
	var usrOrderController = appModule.controller('usrOrderController', function ($scope, ipcService, $http) {
		//订单号
		$scope.orderid = "";
